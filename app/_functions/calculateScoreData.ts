...
export function calculateScoreData(
  ...
  const testResponse1 = testResponse.body.map((testResponseGroup) => {
    const testPaperGroup = testPaper.body.filter(
      (e) => e.groupName === testResponseGroup.groupName
    )[0];

    return testResponseGroup.sections.map((testResponseSection) => {
      const testPaperSection = testPaperGroup.sections.filter(
      ...
      testResponseSection.questions.map((testResponseQuestion) => {
        ...
        switch (questionResult.evaluation) {
          ...
          case Evaluation.Partial:
            ...
            break;
          case Evaluation.Unattempted:
            sectionScoreData.marks.unattempted += 1;
      ...
      const maxMarksMissed = unansweredQMaxMarksList
        .slice(0, unattemptedQuestionsLeft)
        .reduce((a, i) => a + i, 0);

      sectionScoreData.questions.unattempted = unattemptedQuestionsLeft;
      sectionScoreData.marks.unattempted = maxMarksMissed;

      sectionScoreData.questions.total = maxQLimit;
      sectionScoreData.marks.max = sectionScoreData.marks.correct;
    });
  });
}
